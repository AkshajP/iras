...
const FloorList: React.FC<FloorListProps> = ({ selectedFloor, onSelectFloor }) => {
          ...
          <Text fontSize='large'>
            {floors.map(floor => (
              <ListItem marginLeft={10} paddingY={2} key={floor.floor}>
                <Button fontWeight = {floor.floor === selectedFloor?.floor ? 'bold' : 'normal'}
                  fontSize='lg' onClick={() => onSelectFloor(floor)}
                  background = {floor.floor === selectedFloor?.floor ? 'teal.500' : 'gray.700'}
                  width = {150}
                  >
                  {floor.text}
                </Button>
